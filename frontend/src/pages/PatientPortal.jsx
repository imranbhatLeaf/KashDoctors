  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const upcoming = apptsRes.data.data.filter(a => a.status === 'pending' || a.status === 'confirmed').slice(0, 2);
      ...
      <div className="section-padding" style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 22px' }}>
        ...
        {successMessage && (
          <div style={{ 
            ...
            <button onClick={() => setSuccessMessage('')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '20px' }}>&times;</button>
          </div>
        )}
        ...
        {appointments.length > 0 && (
          <div style={{ marginBottom: '60px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              ...
              <button className="button-dark-utility" onClick={() => navigate('/my-appointments')}>View All</button>
            </div>
              ...
              {appointments.map(appt => (
                <div key={appt._id} style={{ 
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p className="caption" style={{ color: 'var(--color-ink-muted-80)' }}>{appt.slotDate} • {appt.slotTime}</p>
                  </div>
                  <span style={{ 
                    fontSize: '10px', 
                    fontWeight: '700', 
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '32px', flexWrap: 'wrap', gap: '24px' }}>
          <div>
            ...
            <p className="body" style={{ color: 'var(--color-ink-muted-80)' }}>Search by name or filter by medical department.</p>
          </div>
        ...
        {!loading && (
          <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '16px', marginBottom: '32px', msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
        ...
        {loading ? (
          ...
          <div style={{ 
            ...
            {filteredDoctors.map((doc) => (
              <DoctorCard key={doc._id} doctor={doc} onBook={() => handleBookClick(doc)} />
            ))}
